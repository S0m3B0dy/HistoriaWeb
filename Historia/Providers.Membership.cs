...
namespace BFC.Framework.Providers
{
    public class Membership : ExtendedMembershipProvider
    {
        ...
        public override void Initialize(string name, System.Collections.Specialized.NameValueCollection config)
        {
            ...
            if (String.IsNullOrEmpty(name))
                name = "HistoriaMembershipProvider";

            if (String.IsNullOrEmpty(config["description"]))
                config["description"] = "Historia Membership Provider for SQL Server";
            
            base.Initialize(name, config);

            connectionString = config["HistoriaConnectionString"];
                ...
                using (SqlCommand cmd = conn.CreateCommand())
                {
                    ...
                    try
                    {
                        ...
                        using (SqlDataReader dr = cmd.ExecuteReader())
                        {
                            ...
                            {
                                Historia.Framework.Security.SecurePassword lastPass = new Historia.Framework.Security.SecurePassword((byte[])dr["Password"]);
                    ...
                    {
                        Historia.Framework.Security.SecurePassword currPass = new Historia.Framework.Security.SecurePassword((byte[])cmd.Parameters["@CurrentPassword"].Value);
        ...
        public override string ResetPassword(string username, string answer)
        {
            ...
            if (!String.IsNullOrEmpty(pass))
            {
                if (SetUserPassword(username, pass, true))
                {
                    ...
                    //com.bfcusa.Messaging.MailMessage mm = new com.bfcusa.Messaging.MailMessage();
        ...
        public override string CreateUserAndAccount(string userName, string password, bool requireConfirmation, IDictionary<string, object> values)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.CommandText = "Membership_InsertUser";
                    cmd.Parameters.Add("@UserName", SqlDbType.NVarChar, 100).Value = userName ?? "";
                    cmd.Parameters.Add("@Password", SqlDbType.VarBinary, 32).Value = Historia.Framework.Utility.sha256Hash(password);
                    cmd.Parameters.Add("@EmailAddress", SqlDbType.NVarChar, 256).Value = values["EmailAddress"].ToString() ?? "";
